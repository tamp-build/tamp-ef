...
namespace Tamp.EFCore.V8.Tests;
...
public sealed class EFCoreTests
{
    ...
    {
        // TAM-84: compare through tool.Executable.Value; Windows rewrites POSIX paths.
    ...
    public void DbContextOptimize_Full_Surface_Round_Trip()
    {
        ...
        Assert.DoesNotContain("--precompile-queries", args);
        Assert.DoesNotContain("--nativeaot", args);
    }
    ...
    public void V8_DbContextOptimize_Has_No_PrecompileQueries_Or_NativeAot_Setters()
    {
        var setters = typeof(EFCoreDbContextOptimizeSettings)
            ...
            .Select(m => m.Name)
            .ToHashSet();
        Assert.DoesNotContain("SetPrecompileQueries", setters);
        Assert.DoesNotContain("SetNativeAot", setters);
    }
